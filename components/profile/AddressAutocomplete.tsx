    ...
    <div ref={wrapperRef} className="relative">
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white placeholder-white/25 outline-none focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 disabled:opacity-60"
        />
        {loading && (
          <svg className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-white/40" fill="none" viewBox="0 0 24 24">
      ...
      {open && suggestions.length > 0 && (
        <ul className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-white/10 bg-[#13132a] py-1 shadow-xl shadow-black/40">
          {suggestions.map((f, i) => (
            <li key={i}>
              <button
                type="button"
                onClick={() => handleSelect(f)}
                className="flex w-full flex-col gap-0.5 px-3.5 py-2 text-left transition-colors hover:bg-white/5"
              >
                <span className="text-sm text-white/80">{f.properties.label}</span>
                <span className="text-[10px] text-white/30">{f.properties.context}</span>
              </button>
            </li>
          ))}
        </ul>
